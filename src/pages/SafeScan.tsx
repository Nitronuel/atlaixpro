...
export const SafeScan: React.FC = () => {
    ...
    const handleScan = async () => {
        ...
        try {
            const nextReport = await SafeScanService.analyzeToken(tokenAddress, chain);
                ...
                <div className="w-full max-w-[760px]">
                    ...
                    <div className="rounded-[30px] border border-border bg-card p-10 text-center shadow-[0_20px_50px_rgba(0,0,0,0.28)]">
                        ...
                        <h2 className="mb-3 text-4xl font-black tracking-tight text-text-light">
                            {loading ? 'Building Safe Scan map' : 'Safe Scan Intelligence'}
                        </h2>
                        <p className="mx-auto max-w-[540px] text-lg leading-9 text-[#9AA8C7]">
                            {loading
                                ? 'Moralis is selecting the top holder wallets, then Alchemy is tracing only those wallets for holder flow, shared funding, and cluster links.'
                                : 'Run a holder-first token safety map for Solana and EVM tokens using Moralis top holders with Alchemy relationship tracing.'}
                        </p>
                    </div>
                </div>
            </div>
